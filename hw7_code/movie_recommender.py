...
def get_train_mse(R, user_vecs, movie_vecs):
    ...
    pred = np.dot(user_vecs, movie_vecs.T)
    mse_loss = np.sum(np.square(np.subtract(pred, R)))
    return mse_loss
...
def update_user_vecs(user_vecs, movie_vecs, R, user_rated_idxs):
    ...
    for i in range(user_vecs.shape[0]):
        user_rated_idx = user_rated_idxs[i]
        # print(movie_vecs[0].shape)
        numerator = np.zeros((movie_vecs[0].shape[0], 1))
        denominator = np.zeros((movie_vecs.shape[1], movie_vecs.shape[1]))
        for j in user_rated_idx:
            movie = movie_vecs[[j]].T
            numerator += R[i, j] * movie
            denominator += np.dot(movie, movie.T)
        denominator += np.identity(denominator.shape[0])
        new_user = np.dot(np.linalg.inv(denominator), numerator)
        user_vecs[i] = new_user.reshape(user_vecs[i].shape)
    return user_vecs
# Part (f): Function to update user vectors
def update_movie_vecs(user_vecs, movie_vecs, R, movie_rated_idxs):
    ...
    for j in range(movie_vecs.shape[0]):
        movie_rated_idx = movie_rated_idxs[j]
        numerator = np.zeros((user_vecs[0].shape[0], 1))
        denominator = np.zeros((user_vecs.shape[1], user_vecs.shape[1]))
        for i in movie_rated_idx:
            user = user_vecs[[i]].T
            numerator += R[i, j] * user
            denominator += np.dot(user, user.T)
        denominator += np.identity(denominator.shape[0])
        new_movie = np.dot(np.linalg.inv(denominator), numerator)
        movie_vecs[j] = new_movie.reshape(movie_vecs[j].shape)
    return movie_vecs

# Part (f): Perform loss optimization using alternating updates
filled_R = np.nan_to_num(R)
train_mse = get_train_mse(np.copy(filled_R), user_vecs, movie_vecs)
...
for opt_iter in range(20):
    ...
    train_mse = get_train_mse(filled_R, user_vecs, movie_vecs)
    train_acc = get_train_acc(np.copy(R), user_vecs, movie_vecs)
    val_acc = get_val_acc(val_data, user_vecs, movie_vecs)
