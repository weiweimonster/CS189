...
def initialize_dataset(
    ...
    if name == 'higgs':
        ...
        train_mean = np.mean(training_set, axis=0, keepdims=True)
        training_set -= train_mean
        training_set = whiten(training_set)

        val_mean = np.mean(validation_set, axis=0, keepdims=True)
        validation_set -= val_mean
        validation_set = whiten(validation_set)

        test_mean = np.mean(test_set, axis=0, keepdims=True)
        test_set -= test_mean
        test_set = whiten(test_set)
